...
//     try {
//       const { cred } = await c.req.json<{ cred: AuthenticationResponseJSON }>();
//       const userHandle = cred.response.userHandle as string;
//       const user = await kv.get<User>(["users", userHandle]);
...
//       const expectedChallengeResponse = await kv.get<string>(["challenges", userHandle]);
//       const expectedChallenge = expectedChallengeResponse.value;
//       const clientDataJSON = JSON.parse(atob(cred.response.clientDataJSON));
      
//       if (!expectedChallenge || expectedChallenge !== clientDataJSON.challenge) {
//         console.error("Challenge mismatch");
//         return c.json({ error: "Unauthorized" }, { status: 401 });
//       }
  
//       const authenticator: AuthenticatorDevice | undefined = user.value?.credentials[cred.id];
//       if (!authenticator) {
...
//       if (user.value) {
//         user.value.credentials[cred.id].counter = verification.authenticationInfo.newCounter;
//         await kv.set(["users", userHandle], user.value);
//       }
  
//       // Issue a new token or session
//       // ...
